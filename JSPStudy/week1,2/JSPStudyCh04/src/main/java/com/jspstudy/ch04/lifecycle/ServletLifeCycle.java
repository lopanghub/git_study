...
@WebServlet(name = "servletLifeCycle",
		// urlPatterns={"*.life"},
		initParams = @WebInitParam(name = "ANNOTATION_PARAM", value = "애노테이션 파라미터"))
public class ServletLifeCycle extends HttpServlet {

	/*
	 * 서블릿 컨테이너에서 서블릿 객체가 생성되고 초기화된 다음에 init() 메소드가 호출 된다. 이 메소드는 서블릿 객체가 초기화될 때 톰캣에
	 * 의해서 딱 한 번 호출되는 메소드 이다. 서블릿 객체가 만들어지고 최조 한 번 실행해야할 작업이 있다면 이 메서드를 이용한다.
	 **/
	public void init() throws ServletException {
		System.out.println("init() 메소드 호출됨");

		/*
		 * web.xml에 서블릿을 등록할 때 servlet 태그 하위에 init-param 태그 안에 param-name에 지정한
		 * "READ_FILE"이라는 파라미터 이름을 찾아서 param-value 태그에 지정한 파라미터 값을 읽어온다.
		 **/
	...
	/*
	 * 서블릿 객체가 소멸되기 직전에 톰캣은 딱 한 번 destroy() 메서드를 호출한다. 서블릿이 소멸될 때 마지막으로 해야 할 작업이 있다면
	 * 이 메서드를 이용한다. 주로 열려있는 스트림을 닫거나, 자원을 해제하는 코드 등을 기술한다.
	 * 
	 * 톰캣이 종료되거나 기존 클래스가 수정되면 톰캣은 자신이 관리하고 있는 서블릿을 소멸시키며 서블릿에 구현되어 있는 destroy() 메서드를
	 * 마지막으로 호출한다.
	 **/
	public void destroy() {
