...
public class FormData01 extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * GET 방식으로 전달되는 파라미터의 값을 읽어온다. 폼 컨트롤 중에서 한 줄 텍스트 입력 상자, 비밀번호 입력 상자, 여러 줄 입력 상자에
		 * 입력된 값이 없으면 요청 파라미터는 존재하지만 데이터가 입력되지 않은 상태이므로 공백 문자열("")을 받게 된다. 하지만 라디오 버튼과
		 * 체크 박스는 사용자가 선택하지 않으면 해당 요청 파라미터는 아예 서버로 전달되지 않기 때문에 null 값을 받게 된다.
		 **/
		...
		/*
		 * view 페이지로 제어를 넘겨 요청에 대한 결과를 출력하기 위해 HttpServletRequest 객체로 부터
		 * RequestDispatcher 객체를 구하여 view/formDataView01.jsp로 포워딩을 하고 있다.
		 **/
		RequestDispatcher rd = request.getRequestDispatcher("view/formDataView01.jsp");
		rd.forward(request, response);

		/*
		 * 모델 데이터를 View 페이지로 전달해 View 페이지에서 HTML 문서 형식으로 응답 데이터를 만들기 때문에 아래와 같이 번거롭게 뷰를
		 * 직접 만들 필요는 없다.
		 **/
		/*
		 ...
		 * out.println("		<title>회원 기본 정보</title>"); out.println("	</head>");
		 * out.println("	<body>"); out.println("		<h2>회원 기본 정보</h2>");
		 ...
		 * out.printf("			직 업 : %s<br/>", job); out.println("	</body>");
		 * out.println("<html>");
		 * 
		 * // 작업이 끝나면 스트림을 닫는다. out.close();
		 */
	}
