...
public class BoardUpdateController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 아래 if 문에서 사용할 변수 선언
		String pass = null, title = null, writer = null, content = null, sNo = null, pageNum = null, fileName = null;
		int no = 0;

		/*
		 * 비밀번호가 맞지 않을 경우 먼저 처리해야 하나 Content-Type에 따라서 파라미터를 읽을 수 있는 객체가 다르기 때문에 각각 처리해야
		 * 한다. Content-Type이 application/x-www-form-urlencoded 일 경우
		 * request.getParameter() 메서드로 파라미터를 읽을 수 있지만 multipart/form-data 일 경우에는 request
		 * 객체를 이용해 파라미터를 읽을 수 없기 때문에 각각 처리하는 코드를 기술해야 한다.
		 **/
		// 요청이 multipart/form-data 일 경우
		if (contentType.contains("multipart/form-data")) {

			/*
			 * cos 라이브러리를 이용한 파일 업로드 구현하기 1. MultipartRequest의 생성자 매개변수에 지정할 데이터를 설정
			 **/
			 ...
			 *
			 * 아래와 같이 MultipartRequest 객체를 생성하면 realPath로 지정한 서버의 디렉토리로 파일이 바로 업로드 된다. 생성자
			 * 인수에 대한 자세한 설명은 게시 글 쓰기 컨트롤러의 주석을 참고하자.
			 **/
			 ...
			 * MultipartRequest를 통해 사용자의 요청 정보에 접근할 수 있다. MultipartRequest 클래스의 주요 메서드는 게시 글
			 * 쓰기 컨트롤러의 주석을 참고하자.
			 **/
			/*
			 * 사용자가 폼에 입력한 데이터 읽어오기 MultipartRequest 객체를 통해 파라미터를 읽어 변수에 저장한다.
			 **/
		...
		if (!isPassCheck) {
			...
			/*
			 * 응답 객체에 연결된 PrintWriter 객체를 이용해 응답 데이터를 전송하고 더 이상 실행할 필요가 없으므로 return 문을 이용해
			 * 현재 메서드를 종료한다.
			 **/
			response.setContentType("text/html; charset=utf-8");
			PrintWriter out = response.getWriter();
		...
		/*
		 * 하나의 게시 글 정보를 저장하는 VO 객체를 생성하고 요청 파라미터로 넘겨받은 데이터를 Board 객체에 저장한다.
		 **/
		...
		/*
		 * 게시 글 수정이 완료된 후 response 내장객체의 sendRedirect() 메서드를 이용해 게시 글 리스트로 Redirect 시킨다.
		 ...
		 * 
		 * 게시 글 수정이 완료된 후 Redirect 시키지 않으면 이 페이지를 새로 고침 하여 재요청 할 때 마다 이미 수정된 게시 글을 계속해서
		 * 수정하려고 하는 문제가 발생한다.
		 * 
