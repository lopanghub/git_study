...
class ScoreEvaluation { 
   ...
   public static void main(String args[]) { 
      while(true) { 
         switch(displayMenu()) { //리턴 받은 1~4로 스위치문 실행
            case 1 : 
               inputRecord(); //학생 성적 입력 (class Student2)
               break; 
            case 2 : 
               deleteRecord(); 
               break; 
      ...
      while(true) { 
         ...
         try { 
	         ...
	         if(!input.equalsIgnoreCase("q")) { 
                  Scanner s2 = new Scanner(input).useDelimiter(","); // ,기준으로 구분

                  record.add(new Student2(s2.next(), s2.next(), s2.nextInt() //Student2 클래스의 생성자 
      ...
      while(true) { 
         ...
         do { 
            try { 
               ...
               if(sort >= 1 && sort <= 3) { //sort 값 1~3 아닐시 예외
                  break; 
               } else { 
                  throw new Exception(); 
               } 
   ...
   static void displayRecord() { 
      ...
      if(length > 0) { 
         for (int i = 0; i < length ; i++) { 
            Student2 student = (Student2)record.get(i); //inputRecord에서 add한 데이터를 get
...
class NameAscending implements Comparator { 
   public int compare(Object o1, Object o2){ 
      if(o1 instanceof Student2 && o2 instanceof Student2){ 
         ...
         int result = (s1.name).compareTo(s2.name); //String.compareTo
         System.out.println("이름 오름차순"+result); // 0 같다 , 1(양수) s1이 크다 (s2보다 사전적으로 뒤다) -1(음수)
         return result; //유니코드 순으로 배열
      } 
      return -1; //-1은 왜하는거지? // 기본값 
   } 
} 
...
class Student2 implements Comparable { 
   ...
   public String toString() { //toString 오버라이딩인데 잘 모르겟다
   ...
   //(189, 11, 2) // diff 8 source [1,8,9] result[11]
   static String format(String str, int length, int alignment) { //정적 메서드format에 대해
      int diff = length - str.length(); 
      if(diff < 0) return str.substring(0, length); //str의 최대길이를 length만큼만 맞추기

      char[] source = str.toCharArray(); //j a v a
      ...
      switch(alignment) { //????? 배열카피를 왜쓰는지 모르겠음
         case CENTER : 
         ...
         default : 
            ...
            //source를 복사, source 0번부터 복사, result에 붙여넣기, result 0번부터 붙여넣기, 소스의 크기 만큼
      } 
